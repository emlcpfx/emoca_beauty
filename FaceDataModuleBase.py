...
class FaceDataModuleBase(pl.LightningDataModule):
    ...
    def _detect_faces_in_image(self, image_or_path, detected_faces=None):
        ...
        for bi, bbox in enumerate(bounding_boxes):
            # Walt added stuff here to expand the bounding box relative to frame width, clamping to min/max of the original image
            expansion = int(w/25)
            left = max(bbox[0] - expansion, 0)
            right = min(bbox[2] + expansion, w)
            top = max(bbox[1] - expansion, 0)
            bottom = min(bbox[3] + expansion, h)
